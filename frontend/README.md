...
- **Bun** — package manager and script runner (`bun.lock`; CI and the Dockerfile both use bun)
- **TanStack React Query + axios** — the API client is **orval-generated** into `src/api/generated.ts` (never edit by hand; regenerate with `bun run codegen`)
- **zod** (client-side email validation), **framer-motion**, **lucide-react**/**react-icons**
...
```sh
bun install
bun run dev        # Vite dev server (no /api proxy — form submissions 404 in standalone dev; use docker compose, or re-add the proxy)
bun run build      # tsc -b && vite build
bun run lint       # oxlint
bun run format     # Prettier
bun run codegen    # re-export the backend OpenAPI spec and regenerate src/api/generated.ts (requires uv)
```
...
├── components/   # layout (header/footer), theme, ui/ (shared UI)
