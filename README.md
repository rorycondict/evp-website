...
3.  Set up your environment variables. 
    * Navigate to the backend directory and duplicate the example file:
    ...
    ```
    * Open the new `.env` file and set `RESEND_API_KEY`. Leave the key commented out for development, requests will be logged instead.
...
* **TanStack React Query + axios:** Server-state management and HTTP client. The API layer is generated by **orval** from the backend's OpenAPI spec (`bun run codegen`).
...
* **Bun:** JavaScript runtime, package manager, and script runner (lockfile: `bun.lock`; used by CI and the Docker build).
...
* **Resend:** Email delivery and contact management.
* **Ruff:** Python linter.
